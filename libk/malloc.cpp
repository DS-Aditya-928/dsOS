...
extern "C" void* malloc(size_t s)
{
    ...
    // UART::print("Number of heap entries: "); UART::print(numEnt);
    // UART::print("\r\n");

    for (int i = 1; i <= numEnt; i++)
    {
        heapMD md = *(heapMD*)(heapStart + heapSize - 2 - (i * sizeof(heapMD)));
        // UART::print(md.regAddr);UART::print("
        // ");UART::print(md.regLength);UART::print("\r\n");
        if ((md.regLength < 0))
        {
            if ((md.regAddr + s) <= (heapSize - 2 - ((numEnt + 1) * sizeof(heapMD)))) // make sure hnew region end wont
                                                                                      // clash with metadatas new entry
            {
                // modify existing md region to account for its shrining and
                // append new region to end.
...
extern "C" void free(void* p)
{
    ...
    unsigned short heapSize = cMMU::getHeapSize();
    unsigned short numEnt = *(unsigned short*)(heapStart + heapSize - 2);
    // UART::print("(free)Number of heap entries: "); UART::print(numEnt);
    // UART::print("\r\n");
...
void getHeapMD()
{
    ...
    unsigned short heapSize = cMMU::getHeapSize();
    unsigned short numEnt = *(unsigned short*)(heapStart + heapSize - 2);
    UART0::print("Number of heap entries: ");
    UART0::print(numEnt);
    UART0::print("\r\n");
    ...
    {
        heapMD md = *(heapMD*)(heapStart + heapSize - 2 - (i * sizeof(heapMD)));
        UART0::print(md.regAddr);
        UART0::print(" ");
        UART0::print(md.regLength);
        UART0::print("\r\n");
    }
}
